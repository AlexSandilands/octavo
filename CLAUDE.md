...
- **Images** — editor uploads (WebP via sharp), served from R2 when configured, else a local-disk fallback (`.data/uploads`) so it works with no cloud setup. A photo can also take the **whole page** (content v6), two ways: **Fill page** covers the canvas edge to edge and trims the photo to the page's shape, **Fit page** grows the photo whole until its first edge touches and centres it against page-coloured bars. Either way it owns the page — the running footer and the theme's page decoration stand down rather than print over it, the caption is dropped (alt text stays), and nothing else may be added to that page. Pressing either on a photo that shares a page moves the photo onto a page of its own in one undoable step; switching between the two is a plain edit. Opt-in and image-only; a cover page ignores both.
...
- **Magazine details** — `settings` table (one row) + `/admin/magazine`: the owner edits the magazine name, club name and tagline, the page-top textual running head, and the page footer's appearance (mark size and text size — Small/Medium/Large presets or a Custom number of px within caps, mark 12–48 / text 8–16 — and alignment), with a live preview built from the real page components. Effective value = DB → `NEXT_PUBLIC_*` env → shipped default, resolved in `src/server/settings.ts`; the env vars are bootstrap defaults only, and a deployment that never opens the page renders exactly as before. No redeploy to change any of it.
- **Issue numbering** — `issues.number` is the unique, immutable `/read/[number]` identity; `displayNumber` is the independently editable reader-facing number. Every visible issue number resolves as `displayNumber ?? number`, including published issues, while reader/email/PDF links keep the identity. Effective displayed numbers are unique. New issues allocate identity from `max(number) + 1` and display from `max(coalesce(displayNumber, number)) + 1`, so a corrected sequence continues without breaking old URLs.
