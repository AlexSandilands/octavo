...
- **Images** — editor uploads (WebP via sharp), served from R2 when configured, else a local-disk fallback (`.data/uploads`) so it works with no cloud setup. A photo can also be set to **fill the page** (content v6): it takes the whole canvas edge to edge, cropped to fill, and owns the page — the running footer and the theme's page decoration stand down rather than print over it, the caption is dropped (alt text stays), and nothing else may be added to that page. Pressing it on a photo that shares a page moves the photo onto a page of its own in one undoable step. Opt-in and image-only; a cover page ignores it.
