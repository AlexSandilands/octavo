...
- **PDF import** — the editor's right-hand side panel (tool rail on the editor's edge, slides in, drag-to-resize) hosts **Import PDF**: a local PDF is parsed in the browser (PDF.js in a same-origin worker, nothing uploaded), its text blocks and photos are drawn as selectable regions over the rendered page, each with a suggested block kind (heading / text / image) that a small tool pill can retype or split; a selection (or _Select all on page_) is added in one measured, undoable step after the selected block, fitting onto continuation pages as needed. Draft issues only. `docs/pdf-import.md` has the limits and the production gates.
