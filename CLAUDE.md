...
- **Editor undo/redo** — the editor keeps a capped (100) snapshot history of the whole document plus the author's place in it, so every structural edit — insert, delete, move, reorder, cover toggle, page add/delete, an overflow split — is undone and redone with one step, landing back on the page and block it happened to. A run of edits to one field (typing, a size nudge, a caption) folds into a single step. Text typed inside a block still undoes finer, through Tiptap and the in-place editors' own history: the editor-level Ctrl/Cmd+Z (Shift+Z / Y to redo) stands down whenever focus is in a text entry. Undo and redo sit in the editor's tool bar, which now floats over the foot of the canvas as one pill with the insert buttons and the cover toggle, clear of the page at the fitted view.
