...
| `logos`                           | The club's own logo marks (issue #92): `name`, `imageId` (→`images`, notNull, cascade on image delete — a logo _is_ its mark), `createdAt`. Managed at `/admin/logos` through the same sharp→WebP→storage pipeline as every other upload, which preserves transparency. Deleting one is refused while it is referenced — the referencing sites are registered in `REFERENCE_COUNTERS` (`src/server/logos.ts`); `issues.logoId` is the first.       |
