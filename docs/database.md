...
[`src/db/schema.ts`](../src/db/schema.ts), with the AI assistant's tables beside it in
[`src/db/schema-ai.ts`](../src/db/schema-ai.ts) (`drizzle.config.ts` lists both); the client in
...
| `ai_usage`                        | One row per AI assistant model request (issue #307): member, issue, run, model, provider, the four token counts (uncached input, cache read, cache write, output) and the priced `cost_usd`. Member and issue are `set null` so spend outlives them. See [AI assistant spend](#ai-assistant-spend-issue-307).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `ai_grants`                       | The owner's one-off budget top-ups (`npm run ai:grant`): amount, note, date. No member column.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
- An admin's retirement sticks (`retired_by = 'admin'`, `adminRetireName`): the member's
  `addName` and `renameName` refuse that name on that account ("An admin has retired this
  name…"), while other accounts may still use it. An admin can also retire a name the member
  had already retired, which makes it stick, and can reword a retired name with
  `adminRenameName` (for what its old comments show); it stays retired, and the block moves
  to the new wording. Names retired before the column existed are `member`.
...
## AI assistant spend (issue #307)

The ledger and budget behind the AI editing assistant (epic #306, [ai-assistant.md](ai-assistant.md)).
Storage and a server module only — no UI and no model calls; the proxy route writes the ledger and
the admin usage page reads it.

**Tables.** `ai_usage` holds one row per request the proxy sends. A run (one author message and
everything the model does in answer) makes several, sharing a `run_id`. Input is recorded as the
provider bills it: `prompt_tokens` is the uncached remainder, `cache_read_tokens` and
`cache_write_tokens` the rest. Cached reads outweighed uncached input by orders of magnitude on every
spike run, at a tenth of the price, so pricing all input at the full rate would overstate spend
several times over. `model` is what the provider reported (a dated id, or with a trailing `~` when
the proxy had to estimate the tokens). `cost_usd` is `numeric(10,6)`, priced when the row is written
and never recomputed. `ai_grants` rows are `numeric(10,2)` dollars with a required note (≤ 200
characters, a CHECK beside `amount_usd > 0`).

**The budget.** A month is a calendar month in **UTC** (`"YYYY-MM"`). `resolveBudget(now)` in
[`src/server/ai-budget.ts`](../src/server/ai-budget.ts) returns, for the month of `now`:

```
allowance = AI_MONTHLY_BUDGET_USD          (env, default 0)
granted   = Σ ai_grants.amount_usd         made in the month
spent     = Σ ai_usage.cost_usd            written in the month
remaining = max(0, allowance + granted − spent)
```

A grant counts only in the month it was made; nothing carries over. A deleted issue's or removed
member's rows stay and keep counting. `runSpend(runId)` sums one run for the per-run cap
(`RUN_SPEND_CAP_USD`, $0.50), and `usageByDay(month)` is the usage page's daily rollup (days with
usage only, oldest first).

**Prices** are code, in [`src/lib/ai-pricing.ts`](../src/lib/ai-pricing.ts): per model, dollars per
million tokens for input, cache read, cache write (the 5-minute rate) and output, each with the date
it was checked. `recordUsage` looks the reported id up exactly, then as a key plus a date snapshot
(`claude-haiku-4-5-20251001`); any other id **throws** before a row is written, so a new `AI_MODEL`
can't run unmetered. `fake` (the gates' provider) is priced at zero. Changing a model means adding
its prices first.

**Grants** are made outside the app with `npm run ai:grant -- <usd> "<note>"`
([infrastructure.md](infrastructure.md#ai-assistant-budget)). `scripts/check-ai-budget.mts` proves the
arithmetic against the local database.
...
1. Edit `src/db/schema.ts` (or `schema-ai.ts`). A new schema file must be added to the
   `schema` list in `drizzle.config.ts` — drizzle-kit only sees the files listed, and a table
   missing from them comes out of the next generated migration as a `DROP TABLE`.
