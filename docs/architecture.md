...
src/
  app/                 routes (App Router). Server components by default.
    ...
    api/admin/ai/render/ pictures of a draft's pages for the assistant (#342)
    api/admin/ai/photo/  one of the issue's photos at 800px, for view_photo (#342)
...
| `/read/draft/[nonce]/print`          | dynamic       | The assistant's draft pictures (issue #342): the draft the render route stashed under the nonce, printed by the same `PrintDocument`. Same internal token and edge-gate carve-out as the PDF print view; 404 without both the token and a live nonce                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `GET /api/admin/ai/usage`            | route handler | The month's assistant spend (issue #309): admin-only (403 JSON), 404 while `AI_PROVIDER` is unset, `{ month, allowance, granted, spent, remaining }` from `resolveBudget()`, no-store. The editor panel's footer and budget-spent state read it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `POST /api/admin/ai/render`          | route handler | Pictures of a draft's pages for the assistant (issue #342): the editor's issue as it holds it now (unsaved edits included, validated by `issueContentSchema`, ≤ the save cap) and up to 8 page numbers. Same gate as the chat route (admin + same origin 403, 404 while off, draft only 409, 120 a 10 minutes per admin 429). Stashes the content in memory under a one-time nonce (60 s) and screenshots `/read/draft/[nonce]/print` in headless Chromium at 960×1350, each page with its measured fill; PNG, or JPEG past the tool result's cap. No-store                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `POST /api/admin/ai/photo`           | route handler | One of the issue's photos for `view_photo` (issue #342): the chat route's gate, then only a photo uploaded to the issue or placed in it, never a logo-library mark (404). Downscaled to 800px on the long edge as a JPEG. No-store                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
