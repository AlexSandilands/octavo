...
| `/admin/magazine`              | dynamic       | Magazine details (issue #105): the owner-editable branding text and page-footer appearance, with a live preview built from the real `PageFrame`/`PageFooter`, over the logo library — named club marks (transparent PNG/WebP) through the shared image pipeline, upload / rename / delete. Delete refuses while the logo is referenced (`countLogoReferences` in `src/server/logos.ts`). `/admin/logos` 308s here                                                                                                                                                                                                                                                           |
