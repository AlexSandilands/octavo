...
## Discussion (issue #301)

Every published issue has a thread — top-level comments and one level of replies, plain
text — that members read and write from the reader. **One component, two shells**:
`DiscussionThread` (`src/features/discussion/`) is the list, its inline reply/edit boxes and
the composer pinned beneath; `DiscussionDrawer` wraps it for the desktop flipbook (floating in
...
```
Reader page (server)  ─ settings.commentsEnabled? ─▶ DiscussionInfo { issueNo, count, signedIn }
                         (count: one countComments query; null for demo's signed-out visitor)
Reader (client)       ─ useDiscussion: open state + a history entry per opening (Back closes)
  shell opens ─▶ GET /api/issues/[number]/comments   (member-gated, no-store)
                   └─ listComments ▸ toThreadEntries ▸ { entries, count, composer }
...
Nothing about a thread is in the issue page's HTML except the count — the list is fetched
only when a shell first opens and after each write, so a closed drawer costs no query. The
...
in the PDF there is no thread at all; demo mode's signed-out visitor gets the control without a
count and a sign-in panel, and the list route answers 401. The library's cards carry "N
...
| `/read/[issueId]`                    | dynamic       | Reader, by issue **number**, published only. **Member session required**. With discussion on it carries the thread's control (issue #301); `?discussion=1[&comment=<id>]` opens the drawer or sheet on load                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
