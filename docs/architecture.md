...
| `/read/[issueId]`              | dynamic       | Reader, by immutable issue **route number**, published only. Visible numbering uses the independently editable `displayNumber`, falling back to the route number for legacy rows. **Member session required**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `GET /api/issues/[number]/pdf` | route handler | **Member session required, and refused with 403 when the owner has switched PDF downloads off** (issue #162 — `settings.pdfDownloads`, checked right after the member check; applies in demo mode too, since it is a distribution choice rather than an auth gate). On-demand PDF addressed by immutable route number: serves cached bytes (`pdfs/{issueId}/{routeNumber}-{displayNumber}-{revision}-{theme}-{logoId}-{chrome}-{sponsors}-v{N}.pdf` — `?theme=` follows the reader's toggle; `{displayNumber}`, `{logoId}`, `{chrome}` and `{sponsors}` are render inputs `revision` doesn't cover (the editable number printed in page chrome and used in the filename; the issue's footer mark; a short hash of the magazine settings that reach a printed page — name, org, footer mark/text size and alignment and the running-head choice, tagline deliberately excluded since it never prints, see `chromeFingerprint` in `src/server/settings.ts`; and a short hash of the managed sponsors this issue's blocks reference as they resolve now — name, href, logo URL and whether the sponsor still exists, so renaming, relinking, relogoing or deleting one re-keys instead of serving a stale PDF, see `sponsorFingerprint` in `src/server/sponsors.ts`); `v{N}` is the code-side render version), else generates via Playwright, caches, serves. Bytes are proxied (not a public URL) so the PDF stays members-only |
...
env as before. The footer look, running-head switch and PDF-download switch have no env var at all —
their defaults are code constants (`DEFAULT_FOOTER_STYLE`, `DEFAULT_SHOW_RUNNING_HEAD`,
`DEFAULT_PDF_DOWNLOADS`), i.e. the behaviour the code shipped with. Note the one asymmetry the download switch introduces: because a read failure
...
The running-head switch is page chrome, not a theme replacement: `PageFrame` passes it into every
theme's decoration on every surface. Classic omits only its magazine-name/issue-number text when
the switch is off and keeps both frame rules; themes without textual heads are unchanged. The
effective choice is part of `chromeFingerprint`, so PDFs regenerate after it changes.
