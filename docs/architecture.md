...
| `GET /api/issues/[number]/pdf` | route handler | **Member session required.** On-demand PDF: serves the cached bytes (`pdfs/{issueId}/{revision}-{theme}.pdf` — `?theme=` follows the reader's toggle), else generates via Playwright, caches, serves. Bytes proxied (not a public URL) so the PDF stays members-only |
