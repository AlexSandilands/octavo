...
| `/read/[issueId]/print`        | dynamic       | Internal print view for PDF generation — every page at full canvas size. **Not session-gated** (the localhost generator has no cookie); guarded by an internal token (`src/lib/pdf-token.ts`), 404 without it. Excluded from the edge gate. Stamps the chrome fingerprint of the settings it resolved into the document (`<meta name="print-chrome">`); the generator refuses to return a PDF whose stamp isn't the one the cache key names, so a settings edit or a database blip between the two requests fails the download instead of caching wrongly-branded bytes (issue #127)                                                                                        |
...
| `GET /api/issues/[number]/pdf` | route handler | **Member session required.** On-demand PDF: serves the cached bytes (`pdfs/{issueId}/{revision}-{theme}-{logoId}-{chrome}-v{N}.pdf` — `?theme=` follows the reader's toggle; `{logoId}` and `{chrome}` are the two render inputs `revision` doesn't cover (the issue's footer mark, and a short hash of the magazine settings that reach a printed page — name, org, footer mark/text size and alignment, tagline deliberately excluded since it never prints; see `chromeFingerprint` in `src/server/settings.ts`); `v{N}` is the code-side render version), else generates via Playwright, caches, serves. Bytes proxied (not a public URL) so the PDF stays members-only |
...
plus the running footer's mark size, type size and alignment — live in the single-row `settings`
