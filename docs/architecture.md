...
| `/admin/members`               | dynamic       | Members CRUD on the `users` table: add / remove / toggle subscribed / toggle admin / CSV import (guard rails: no self-removal, keep one admin). List is paginated (25/page) with a DB-side search; both live in the URL (`?q=`, `?page=`) so the view survives refresh and `revalidatePath`                                                                                                                                                                                                                                                                                                                                                                                 |
