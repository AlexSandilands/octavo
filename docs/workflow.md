...
| The member profile / avatar route     | `npx tsx --tsconfig scripts/tsconfig.json scripts/dev-profile-gate.mts <base-url>` and `dev-asset-lifecycle-gate.mts <base-url>` — names, avatars and their refusals, email, the admin dialog (#300)                                                          |
| The assistant's tools and runs        | `npx tsx --tsconfig scripts/tsconfig.json scripts/check-ai-tools.mts`, `… check-ai-runs.mts`, and `… dev-assistant-tools-gate.mts <base-url>` on a fake-provider dev server — tools, refusals, split, one step per run, hands off, presets, breakers (#310)   |
| The AI budget / pricing module        | `npx tsx --tsconfig scripts/tsconfig.json scripts/check-ai-budget.mts` — the month's arithmetic, grants and their refusals, cache-rate pricing, dated/estimated/unknown ids, run spend, the daily rollup, UTC boundaries; own rows only (#307)                |
| Discussion storage / module           | `npx tsx --tsconfig scripts/tsconfig.json scripts/check-member-name.mts` and `… scripts/check-comments-module.mts` (+ `check-report-moderation.mts`) — name rules, refusals, read shapes, reports, removal policies, avatars; own rows only (#299)            |
| The reports inbox / moderation        | `npx tsx --tsconfig scripts/tsconfig.json scripts/dev-reports-gate.mts <base-url> [shots]` — filters, search, every row action, the counts, keyboard, the discussion settings and both removal confirmations (#302)                                           |
| The reader's discussion thread        | `npx tsx --tsconfig scripts/tsconfig.json scripts/dev-discussion-gate.mts <base-url>` — drawer, sheet, posting, deep links, off switch, counts, moderation, tags; run again against a `NEXT_PUBLIC_DEMO_MODE=1` server for the signed-out visitor (#301–#304) |
| Reply notifications (bell, email)     | `npx tsx --tsconfig scripts/tsconfig.json scripts/dev-notifications-gate.mts <base-url> <dev-log>` — bell, reply email, magic link, both unsubscribe purposes; the bell's keys in `dev-menu-focus-gate.mts`, tokens in `dev-auth-check.mts` (#303)            |
| Issue transfer (export/import)        | `npx tsx --tsconfig scripts/tsconfig.json scripts/check-issue-transfer.mts` and `… scripts/dev-issue-transfer-gate.mts <base-url>` — the round trip, per-use image resolution, library matching, every refusal, cleanup/recovery and the real workflow (#293) |
...
| An admin mutation flow / list refresh | `npx tsx --tsconfig scripts/tsconfig.json scripts/prod-action-refresh-gate.mts <base-url>` against a **production build** (`npm run build` + `next start`) — and that create opens the editor, the draft listed on Back (#198, #276, #296)                    |
| The editor autosave path / save API   | `node --env-file=.env --import tsx scripts/prod-editor-save-gate.mts` after `npm run build`, local DB only — the build-A editor must keep saving through a cold build B, plus conflicts, validation and Retry (#245)                                          |
...
The issue-transfer gate creates its own admin, images, sponsor, logo and issue, adopts
every row an import writes and removes all of them; it refuses to run against configured
R2. Its two size cases — a bundle over 10 MB (Next's proxy buffering limit) and one near
a representative upper bound — run against a **production build** with the local S3
substitute, per the header of `scripts/prod-issue-transfer-size.mts`. `next start` will
not boot in production without `R2_*`/`EMAIL_API_KEY`, which is why those gates use the
fake values and the loopback preloader documented in `docs/pdf-import.md`.

The Members responsive gate runs with
...
and issue-05 keeps the deliberate legacy fixture. Validate them through the gate,
never by running the destructive seed.
