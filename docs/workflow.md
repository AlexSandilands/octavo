...
| Sign-in rate limits / client address  | `npx tsx --tsconfig scripts/tsconfig.json scripts/check-client-ip.mts` — which headers count with and without `ORIGIN_AUTH_SECRET`; then `dev-reader-gate.mts <base-url> <dev-log>`                                                                           |
