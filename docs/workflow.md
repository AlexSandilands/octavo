...
| An admin mutation flow / list refresh | `npx tsx --tsconfig scripts/tsconfig.json scripts/prod-action-refresh-gate.mts <base-url>` against a **production build** (`npm run build` + `next start`) — and that create opens the editor and the new draft survives a Back to the dashboard (#198, #276, #296) |
