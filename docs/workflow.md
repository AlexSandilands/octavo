...
| The reader's discussion thread        | `npx tsx --tsconfig scripts/tsconfig.json scripts/dev-discussion-gate.mts <base-url>` — drawer, sheet, posting, deep links, off switch, counts, moderation; run again against a `NEXT_PUBLIC_DEMO_MODE=1` server for the signed-out visitor (#301, #302)      |
