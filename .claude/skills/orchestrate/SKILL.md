...
description: >
  ...
  issues) stack the branches in an integration worktree and hand Alex a running
...
diff, and give or withhold sign-off. Alex merges (unless they explicitly ask
...
post an explicit "orchestrator sign-off" comment and tell Alex.
...
2. Merge the branches in — smallest / most-conflicting first. Resolve
   ...
   judgement-heavy for Alex. (Prevention beats resolution: when two branches
...
and keep the integration worktree + server only until Alex is done testing.

## Throughout

- Keep Alex updated in short notes: each PR opened, each review verdict, each
  sign-off. Lead with what happened, not process.
- Anything only a human can do (merges by default, accounts, DNS, deploys)
  goes to Alex, not around them.
