...
// `bg-sheet`/`text-lead` utility resolves to the active brand's tokens with no
...
// the two the mark needs are mirrored here from each brand's chrome palette
// (globals.css @theme for heritage, brands.css for the rest): `bg` is the
// brand's signature colour (`--color-brand`), `fg` the paper the monogram sits on
// (`--color-sheet`). Only these two per brand, changed only if that identity
// shifts — keep in step with the tokens.
export const BRAND_ICON_COLORS: Record<BrandId, { bg: string; fg: string }> = {
  heritage: { bg: "#8a1c2b", fg: "#ffffff" },
  coastal: { bg: "#1f3a5f", fg: "#ffffff" },
};
