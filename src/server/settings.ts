...
const storedSchema = z.object({
  ...
  // Postgres types both booleans, so `.catch(null)` only fires on a row that
  // isn't the shape the driver promised. Same treatment anyway: unreadable means
  // "not configured", which is the shipped default — see DEFAULT_PDF_DOWNLOADS
...
// bake the magazine name (classic running head, no-logo footer), whether that
// running head prints at all (issue #269), the org name (footer lockup) and the
// footer's appearance into every page, none of which bumps `issues.revision` —
// so without this a branding edit would serve stale PDFs forever.
//
