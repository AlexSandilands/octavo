...
// The one way an admin list writes its state (?q=, ?filter=, ?page=) into the
// URL — search, filter and pagination all funnel through here so the contract
// lives once instead of drifting three ways. A `null` value removes
