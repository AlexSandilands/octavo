...
export function ImportPreview({
  ...
  return (
    <div className="bg-surface-2 mt-4 rounded-field px-4 py-3 font-ui text-[15px]">
      <div className="text-fg font-bold">{fileName}</div>
      <ul className="text-fg-muted mt-1.5 space-y-0.5">
        <li>
      ...
      {tooMany && (
        <p className="text-warn mt-2 leading-relaxed font-bold">
      ...
      {sample.length > 0 && (
        <div className="border-hairline mt-3 border-t pt-2.5">
          <div className="text-fg-muted text-[13px] font-bold tracking-[0.08em] uppercase">
            {rest > 0 ? `First ${sample.length} of them` : "What we read"}
          </div>
          <ul className="mt-1.5 space-y-1">
            {sample.map((member) => (
              <li key={member.email} className="flex flex-wrap gap-x-2">
                <span className="text-fg">{member.email}</span>
                {member.name ? (
                  <span className="text-fg-muted">{member.name}</span>
                ) : (
