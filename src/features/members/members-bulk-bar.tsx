...
export function MembersBulkBar({
  ...
  // table's state, not ours, so it can only be read in the render that
  // carries the landed selection (adjusted here, during render); the sentence
  // is then written once and left alone, because a line rebuilt from the
  // running count would announce again on every tick of every row checkbox.
  ...
  const countText = (
    <>
      <span className={active ? "text-lead font-semibold" : undefined}>
  ...
  return (
    ...
    >
      ...
      {shownCount > 0 ? (
        ...
        active && (
          <span className="text-grey py-2.5 pl-2 font-ui text-[14px]">
            {countText}
          </span>
        )
      )}
      ...
      <p
        aria-live="polite"
        className={`text-grey-soft basis-full px-1 font-ui text-[14px] ${
          atCap ? "pb-2" : ""
        }`}
      >
        {atCap &&
        ...
        className={
          selectionNote
            ...
            : `basis-full px-1 font-ui text-[14px] ${
                error ? "text-red pb-2" : result ? "text-grey pb-2" : ""
              }`
        }
      >
        {error}
        {!error && result && (
          <>
            Done — <strong className="text-lead">{result.head}</strong>
            {result.tail}
          </>
        )}
        {selectionNote}
