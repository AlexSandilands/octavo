    ...
    <div className="flex flex-col gap-1.5">
      <div className="flex flex-wrap items-center gap-3">
        ...
        {custom && (
          <label
            htmlFor={id}
            className="text-fg flex items-center gap-2 font-ui text-[15px] font-bold"
          >
            Pixels
            <input
              id={id}
              ...
              aria-invalid={!valid}
              className={`text-fg bg-surface h-12 w-24 rounded-field border-[1.5px] px-3 font-ui text-[16px] tabular-nums outline-none ${
                valid ? "border-edge focus:border-primary" : "border-danger"
              }`}
            />
          </label>
        )}
      ...
      {custom && (
        <p
          ...
          className={`font-ui text-[15px] leading-relaxed ${
            valid ? "text-fg-muted" : "text-danger font-bold"
          }`}
        >
          {valid
