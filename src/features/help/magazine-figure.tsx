...
function CardTitle({ n, title }: { n: number; title: string }) {
  return (
    <div className="flex items-center gap-2">
      <FigureBadge n={n} />
      <span className="text-fg font-ui font-bold text-[13px] leading-none">
        {title}
      </span>
    </div>
  );
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <span className="text-fg-muted block font-ui text-[8px] font-semibold tracking-[0.14em] uppercase">
        {label}
      </span>
      <span className="border-hairline text-fg mt-1 flex h-[22px] items-center truncate rounded-full border bg-surface px-2 font-ui text-[10px]">
        {value}
      </span>
    </div>
  );
...
function SettingsColumn() {
  return (
    <div className="flex min-w-0 flex-1 flex-col gap-3">
      <div className="border-hairline bg-surface rounded-field border p-3.5">
        ...
        <div className="border-hairline mt-3.5 border-t pt-3">
          ...
          <div className="mt-2.5 flex flex-wrap gap-1.5">
            {FOOTER_CHOICES.map((c) => (
              <span
                key={c.label}
                className="border-hairlineline text-fg flex h-[23px] items-center gap-1 rounded-full border bg-surface px-2 font-ui text-[9.5px] font-medium"
              >
                {c.label}: {c.value}
        ...
        <div className="border-hairline mt-3.5 flex flex-wrap items-center gap-2 border-t pt-3">
          <FigureBadge n={5} />
          <span className="bg-primary text-surface rounded-full px-2.5 py-1.5 font-ui text-[10.5px] font-semibold">
            Save changes
          </span>
          <span className="text-fg-muted font-ui text-[9.5px]">
            Unsaved changes.
          </span>
        </div>
      </div>
        ...
        <div className="mt-2.5 space-y-1.5">
          {["Club crest", "Wordmark"].map((name) => (
            ...
            >
              ...
              <span className="text-fg flex-1 truncate font-ui text-[10px] font-semibold">
                {name}
              </span>
              <Icon name="pencil" size={11} className="text-fg-muted" />
              <Icon name="trash" size={11} className="text-fg-muted" />
            </span>
          ))}
        </div>
        <span className="bg-primary-soft text-primary mt-2.5 inline-block rounded-full px-2 py-0.5 font-ui text-[9px] font-semibold">
          Saves straight away
        </span>
      </div>
    </div>
    ...
    <div className="flex w-[190px] flex-none flex-col items-center gap-2 sm:w-[200px] lg:w-[240px]">
      <div className="flex w-full flex-wrap items-center gap-1.5">
        ...
        {["Theme: Classic", "Mark: Club crest"].map((c) => (
          <span
            key={c}
            className="border-hairlineline text-fg flex h-[21px] items-center gap-1 rounded-full border bg-surface px-1.5 font-ui text-[9px] font-medium"
          >
            {c}
      ...
      <div className="bg-page flex aspect-[640/900] w-full flex-col rounded-[2px] p-3 shadow-[0_2px_8px_rgba(20,32,28,0.18)]">
        <div className="text-fg-muted font-ui text-[7px] tracking-[0.12em] uppercase">
          Seaview Notes · No. 12
        </div>
        ...
        <div className="text-fg-muted mt-auto flex items-center justify-between font-ui text-[8px] font-medium tracking-[0.1em] uppercase">
      ...
      <div className="flex w-full items-start justify-between px-3">
        <span className="text-fg-muted flex flex-col items-center gap-0.5">
      ...
      <div className="mx-auto flex max-w-[620px] flex-col items-center gap-4 sm:flex-row sm:items-start sm:gap-0">
        ...
        <div className="hidden w-8 flex-none self-stretch sm:flex sm:justify-center">
          <span className="bg-line relative w-px">
            <span className="border-hairlineline absolute top-1/2 left-1/2 h-8 w-[6px] -translate-x-1/2 -translate-y-1/2 rounded-full border-[1.5px] bg-surface" />
          </span>
        </div>
        <PreviewColumn />
      </div>
