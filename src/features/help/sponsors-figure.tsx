...
function MiniPage({ issueNo, badge }: { issueNo: number; badge?: boolean }) {
  return (
    <div className="bg-page w-[126px] rounded-[2px] p-2.5 shadow-[0_2px_8px_rgba(20,32,28,0.18)]">
      <div className="text-fg-muted font-ui text-[8px] tracking-[0.12em] uppercase">
        Issue No. {issueNo}
      </div>
      ...
      <div className="border-hairlineline mt-2 flex items-center gap-1.5 rounded-[4px] border bg-surface p-1.5">
        {badge && <FigureBadge n={2} />}
        <span className="photo-fill h-4 w-4 flex-none rounded-[2px]" />
        <span className="text-fg font-ui text-[8px] leading-tight font-semibold">
          Harbour Bakery
        </span>
      </div>
  ...
  return (
    <FigureFrame caption="One sponsor record, reused everywhere it appears. The numbers match the steps below.">
      <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-center sm:gap-4">
        ...
        <div className="border-hairline w-[190px] flex-none rounded-field border bg-surface p-4">
          <div className="flex items-center gap-2">
            <FigureBadge n={1} />
            <span className="text-fg-muted font-ui text-[8.5px] font-semibold tracking-[0.14em] uppercase">
              Sponsor record
            </span>
          </div>
          <span className="photo-fill mt-2.5 block h-10 w-10 rounded-[4px]" />
          <div className="text-fg mt-2 font-ui text-[13px] font-semibold">
            Harbour Bakery
          </div>
          <div className="text-primary font-ui text-[11px] underline">
            harbourbakery.nz
          </div>
          <div className="text-fg-muted mt-1.5 font-ui text-[10px]">
        ...
        <div className="flex flex-none items-center gap-1.5 sm:flex-col">
          ...
          <Icon
            name="arrowDown"
            size={16}
            className="text-fg-muted sm:hidden"
          />
          <Icon
            name="arrowRight"
            size={16}
            className="text-fg-muted hidden sm:block"
          />
        </div>
