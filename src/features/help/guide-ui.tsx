...
export function GuideSection({
  ...
  return (
    <section
      ...
      className="border-hairline mt-10 scroll-mt-8 border-t pt-8"
    >
      ...
      <h2
        id={`${id}-heading`}
        className="text-lead mt-1.5 font-display text-[26px] leading-tight"
      >
        {title}
      </h2>
...
export function P({ children }: { children: ReactNode }) {
  return (
    <p className="text-lead max-w-[64ch] font-ui text-[15.5px] leading-relaxed">
      {children}
    </p>
  );
}
...
export function Bullets({ children }: { children: ReactNode }) {
  return (
    <ul className="text-lead max-w-[64ch] list-disc space-y-2.5 pl-5 font-ui text-[15px] leading-relaxed marker:text-grey-soft">
      {children}
    </ul>
  );
}
...
export function Step({
  ...
  return (
    <li className="flex gap-3.5">
      ...
      <span
        aria-hidden="true"
        className="bg-newsprint text-red mt-0.5 flex h-7 w-7 flex-none items-center justify-center rounded-full font-ui text-[13px] font-semibold"
      >
        {n}
      </span>
      <div className="min-w-0">
        <h3 className="text-lead font-ui text-[15.5px] font-semibold">
          {title}
        </h3>
        <div className="text-lead mt-1 font-ui text-[15px] leading-relaxed">
          {children}
        </div>
      </div>
    </li>
  ...
  return (
    <div
      className={`max-w-[64ch] rounded-[10px] border-[1.5px] p-4 sm:p-5 ${
        careful ? "border-red bg-newsprint" : "border-hairline bg-sheet"
      }`}
    >
      <h3
      ...
      <div className="text-lead mt-1.5 font-ui text-[14.5px] leading-relaxed">
        {children}
      </div>
    </div>
  );
...
export function FigureFrame({
  ...
  return (
    ...
    <figure className="xl:-mx-24">
      <div
        aria-hidden="true"
        className="border-hairline bg-sheet overflow-hidden rounded-[10px] border p-4 sm:p-6"
      >
        {children}
      </div>
      <figcaption className="text-grey-soft mt-2 font-ui text-[13px] leading-relaxed xl:px-24">
        {caption}
      </figcaption>
    </figure>
  );
