...
const REFUSED =
  "The assistant can’t take a request right now. The panel says why.";
...
export function AskControl({
  ...
  /** False when the conversation can't take it (full, spent, busy). */
  ...
  const submit = () => {
    ...
    if (!onSend(value)) return setRefused(true);
    setValue("");
    setRefused(false);
    setOpen(false);
  };
