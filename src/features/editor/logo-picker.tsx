  ...
  if (logos.length === 0 && !logoId) {
    return (
      <Link
        href="/admin/magazine"
        target="_blank"
        className="text-primary font-ui text-[14px] font-bold underline underline-offset-2"
      >
        Add a logo
      </Link>
    );
...
function MarkChip({ src, name }: { src?: string; name?: string }) {
  return (
    <>
      <span className="border-hairline bg-surface flex h-[22px] w-[30px] flex-none items-center justify-center overflow-hidden rounded border">
        {src && (
