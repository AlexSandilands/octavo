...
export function SiteFooter({
  ...
  return (
    <footer className="border-line text-faint2 mt-4 flex flex-col gap-3 border-t py-8 font-ui text-[13px] sm:flex-row sm:items-center sm:justify-between">
      <div className="text-muted font-display text-[15px]">{org}</div>
      ...
      <div className="flex items-baseline gap-5">
        ...
        {signedIn && (
          <>
            <Link
              href="/preferences"
              className="text-muted hover:text-accent flex h-11 items-center font-ui text-sm font-medium whitespace-nowrap hover:underline"
            >
              Email preferences
            </Link>
            <SignOutButton />
