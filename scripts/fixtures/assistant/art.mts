// The model-selection fixture's pictures (#315), none of them real: the seed's
// procedural art regenerated from its manifest, fresh labelled art for cases
// that build a new issue, and a simple club mark. Real photos come only from
// `--photos <dir>` at run time, for cases that accept them, never committed.
// Files are cached under the git-ignored scripts/assistant-models/.cache.
import { createHash } from "node:crypto";
import { existsSync, mkdirSync, readdirSync, writeFileSync } from "node:fs";
import { join } from "node:path";
import sharp from "sharp";
import { renderArtSvg, type ArtStyle } from "../../../src/db/seed/art.ts";
import { SEED_IMAGES } from "../../../src/db/seed/images.ts";
...
/** Every seed image, as `img-<key>`, rendered as the seed renders it. */
export async function seedArt(): Promise<Map<string, FixtureImage>> {
  const dir = dirFor("seed");
  const out = new Map<string, FixtureImage>();
  for (const spec of SEED_IMAGES) {
    const file = join(dir, `${spec.key}.webp`);
    if (!existsSync(file))
      writeFileSync(
        file,
        await sharp(Buffer.from(renderArtSvg(spec)))
          .webp({ quality: 82 })
          .toBuffer(),
      );
    const id = `img-${spec.key}`;
    out.set(id, { id, width: spec.width, height: spec.height, file });
  }
  return out;
}
...
/** "PHOTO · <role>" in one corner, so a model with vision can tell them apart. */
...
/** A case's new photos: labelled art at the given sizes. */
export async function generatedArt(
  specs: GeneratedSpec[],
): Promise<FixtureImage[]> {
  const dir = dirFor("generated");
  const out: FixtureImage[] = [];
  for (const [i, spec] of specs.entries()) {
    const file = join(dir, `${spec.id}.webp`);
    if (!existsSync(file)) {
      // The first is the cover: a landscape wash, like the seed's covers.
...
/**
 * Every photo in `dir`, for a case that accepts them. Ids are opaque (hashed
 * from the case and index) and sorted, so neither the file names nor their
 * order say what a photo shows.
 */
export async function loadPhotos(
  dir: string,
  caseId: string,
): Promise<FixtureImage[]> {
  const out = dirFor(join("photos", caseId));
  const files = readdirSync(dir)
    .filter((f) => /\.(jpe?g|png|webp)$/i.test(f))
    .sort();
  const photos: FixtureImage[] = [];
...
export async function clubMark(
  ...
  const file = join(dirFor("generated"), `logo-${slug}.webp`);
  ...
  if (!existsSync(file))
    writeFileSync(
      ...
      await sharp(
        Buffer.from(
          `<svg xmlns="http://www.w3.org/2000/svg" width="240" height="240" viewBox="0 0 240 240">${ring}${art}</svg>`,
        ),
      )
        .webp({ quality: 90 })
        .toBuffer(),
